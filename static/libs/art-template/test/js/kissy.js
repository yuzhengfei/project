...
(function (S, undefined) {
    ...
    S.mix(S, {
         ...
         * @return {Object} a timer object. Call the cancelSign() method on this object to stop
         *         the timer.
         */
...
KISSY.add('dom/create', function(S, DOM, UA, undefined) {
        ...
        S.mix(DOM, {
            ...
            /**
             * Creates a new HTMLElement using the provided index string.
             */
            ...
            _creators: {
                div: function(html, ownerDoc) {
                    ...
                    // index 为 <style></style> 时不行，必须有其他元素？
...
//            return 0;
//        }
//        var index = doc.documentElement;
//        if (index === elem) {
//            return true;
//        }
//        else if (DOM.__contains(index, elem)) {
//            return true;
//        }
//        return false;
//    }
...
//                (isStrict ? d[DOC_ELEMENT][CLIENT + name] : d[BODY][CLIENT + name]);
    ...
    function getClientPosition(elem) {
        ...
        if (elem[GET_BOUNDING_CLIENT_RECT]) {
            ...
            // 但测试发现，这样反而会导致当 index 和 body 有边距/边框样式时，获取的值不正确
            // 此外，ie6 会忽略 index 的 margin 值，幸运地是没有谁会去设置 index 的 margin
            ...
            // 最好禁止在 body 和 index 上边框 ，但 ie < 9 index 默认有 2px ，减去
            // 但是非 ie 不可能设置窗口边框，body index 也不是窗口 ,ie 可以通过 index,body 设置
            // 标准 ie 下 docElem.clientTop 就是 border-top
            // ie7 index 即窗口边框改变不了。永远为 2
            ...
            if (oldDisplay === "none" || oldDisplay === "") {
                ...
                if (!defaultDisplayDetectIframeDoc || !defaultDisplayDetectIframe.createElement) {
                    ...
                    try {
                        ...
                        defaultDisplayDetectIframeDoc.write(( doc.compatMode === "CSS1Compat" ? "<!doctype index>" : "" )
                            + "<index><head>" +
                    ...
                    if (val === AUTO) {
                        ...
                        if (S.inArray(DOM.css(elem, 'position'), ['absolute','fixed'])) {
                            ...
                            if (isIE && document['documentMode'] != 9 || UA['opera']) {
                                // 类似 offset ie 下的边框处理
                                // 如果 offsetParent 为 index ，需要减去默认 2 px == documentElement.clientTop
...
KISSY.add('event/hashchange', function(S, Event, DOM, UA) {
    ...
    if ((!( 'on' + HASH_CHANGE in window)) || ie && ie < 8) {
        ...
        var POLL_INTERVAL = 50,
            win = window,
            IFRAME_TEMPLATE = "<index><head><title>" + (doc.title || "") +
                " - {hash}</title>{head}</head><body>{hash}</body></index>",
...
KISSY.add("node/base", function(S, DOM, undefined) {
    ...
    S.mix(NodeList, {
        /**
         * 查找位于上下文中并且符合选择器定义的节点列表或根据 index 生成新节点
         * @param {String|HTMLElement[]|NodeList} selector index 字符串或<a href='http://docs.kissyui.com/docs/html/api/core/dom/selector.html'>选择器</a>或节点列表
         * @param {String|Array<HTMLElement>|NodeList|HTMLElement|Document} [context] 上下文定义
         * @returns {NodeList} 节点列表对象
         */
        all:function(selector, context) {
            // are we dealing with index string ?
...
KISSY.add('anim/easing', function() {
    ...
    Easing.NativeTimeFunction = {
        ...
        //  3. assets/cubic-bezier-timing-function.index
        // 注：是模拟值，非精确推导值
        ...
        var isLocal = rlocalProtocol.test(curLocationParts[1]),
            ...
            defaultConfig = {
                ...
                /*
                 ...
                 timeout: 0,
                 data: null,
                 // 可取json | jsonp | script | xml | index | text | null | undefined
                 dataType: null,
                 username: null,
                 password: null,
                 cache: null,
